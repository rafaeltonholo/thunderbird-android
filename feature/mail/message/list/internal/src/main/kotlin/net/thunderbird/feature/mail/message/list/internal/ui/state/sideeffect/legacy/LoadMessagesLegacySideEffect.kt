...
internal class LoadMessagesLegacySideEffect(
    ...
    override fun accept(event: MessageListEvent, newState: MessageListState): Boolean =
        newState is MessageListState.LoadingMessages

    override suspend fun handle(oldState: MessageListState, newState: MessageListState) {
        if (newState !is MessageListState.LoadingMessages) return
        running?.cancel()
    ...
    class Factory(
        ...
        override fun create(
            ...
            dispatch: suspend (MessageListEvent) -> Unit,
        ): StateSideEffectHandler<MessageListState, MessageListEvent> = LoadMessagesLegacySideEffect(
            scope = scope,
            logger = logger,
